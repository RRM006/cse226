...
def print_transfer_report(report):
    ...
    if report['transferred_courses']:
        for i, course in enumerate(report['transferred_courses'], 1):
            connector = "└─" if i == len(report['transferred_courses']) else "├─"
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("Usage: python transfer_handler.py <transcript.csv>")
        sys.exit(1)
    ...
    program = None
    for r in records:
        if r['code'].startswith('LLB'):
            program = 'LLB'
            break
        elif r['code'].startswith('CSE'):
            program = 'BSCSE'
            break
    ...
    report = create_transfer_report(records, program)
    print_transfer_report(report)
