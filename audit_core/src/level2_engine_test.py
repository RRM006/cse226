#!/usr/bin/env python3
"""
Level 2: CGPA Calculator with Waiver Handler (Non-interactive version for testing)
...
class Level2Engine(AuditCore):
    ...
    def generate_report(self, transcript_file, waivers=None):
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python level2_engine_test.py <transcript.csv> [waiver1,waiver2,...]")
        sys.exit(1)
    ...
    if len(sys.argv) > 2:
        waivers = sys.argv[2].split(',')
    
    engine = Level2Engine()
    engine.generate_report(transcript_file, waivers)
