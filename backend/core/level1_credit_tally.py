...
def run_level1(csv_text: str, program: str, waivers: list[str] = None) -> dict:
    ...
    try:
        ...
        category_results = compute_category_credits(resolved, prog)
        print_output(student_id, prog, "API Upload", records, resolved, retake_info, excluded, category_results)
        
        result_text = sys.stdout.getvalue()
        
        total_credits = sum(r['credits'] for r in resolved.values())
        ...
        result_json = {
            ...
            "eligible": False,
            "missing_courses": [],
            "excluded_courses": [r['code'] for r, _ in excluded],
            "waivers_applied": waivers or []
        }
    finally:
