...
def parse_external_transcript(csv_text):
    """Parse external university transcript from CSV text string."""
    student_id = 'Unknown'
    university = 'Unknown'
    records = []
...
def print_transfer_report(student_id, university, program, result):
    ...
    if result['transferred']:
        for i, t in enumerate(result['transferred']):
            connector = "L " if i == len(result['transferred']) - 1 else "+ "
    ...
    if result['rejected']:
        for i, r in enumerate(result['rejected']):
            connector = "L " if i == len(result['rejected']) - 1 else "+ "
...
def evaluate_external_transfer(csv_text, target_program):
    """Main API function: evaluate external transfer from CSV text string."""
    student_id, university, records = parse_external_transcript(csv_text)
