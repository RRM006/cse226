...
def parse_knowledge_file(filepath, program):
    """
    Parse a program_knowledge.md file into a structured dict.
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python src/level3_audit_engine.py <transcript.csv> <program_knowledge.md>")
        sys.exit(1)
    ...
    if not os.path.isfile(knowledge_path):
        print(f"Error: File '{knowledge_path}' not found.")
        sys.exit(1)

    filename = os.path.basename(transcript_path)
    knowledge_file = os.path.basename(knowledge_path)

    student_id, program, records = parse_transcript(transcript_path)
    knowledge = parse_knowledge_file(knowledge_path, program)
    ...
    audit = run_audit(resolved, retake_info, knowledge, program, records, waivers)
    # Store capstone codes set for display
    audit['missing_capstone_codes'] = set(c['code'] for c in audit['missing_capstone'])

    print()
    print_audit_output(student_id, program, filename, knowledge_file, audit, knowledge)
