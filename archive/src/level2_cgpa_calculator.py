...
def prompt_waivers(program):
    """Prompt admin for waiver courses and validate against program rules."""
    allowed = ALLOWED_WAIVERS.get(program, [])
    raw = input("Enter waived courses (comma-separated, or NONE): ").strip()

    if raw.upper() == 'NONE' or raw == '':
        return []

    requested = [c.strip().upper() for c in raw.split(',')]
    valid_waivers = []
    for code in requested:
        if code in allowed:
            valid_waivers.append(code)
        else:
            print(f"  ⚠ Warning: {code} is not waivable for this program. Ignoring.")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python src/level2_cgpa_calculator.py <transcript.csv>")
        sys.exit(1)
    ...
    # Print header before waiver prompt
    print(f"=== NSU AUDIT CORE - LEVEL 2 ===")
    print(f"Student: {student_id}")
    print(f"Program: {PROGRAM_LABELS.get(program, program)}")
    print(f"Processing: {filename}")
    print()
    ...
    # Waiver prompt
    waivers = prompt_waivers(program)

    # Remove waived courses from resolved if they weren't taken
    # (waivers count toward degree but don't appear in transcript)

    # Calculate CGPA
    cgpa, total_points, total_credits = calculate_cgpa(resolved, waivers)

    # Print output (without re-printing the header)
    print_cgpa_output(student_id, program, filename, records, resolved, retake_info,
                      waivers, cgpa, total_points, total_credits)
